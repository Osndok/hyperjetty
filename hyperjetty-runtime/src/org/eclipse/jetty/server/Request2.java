...
public class Request2 implements HttpServletRequest
{
    ...
    /* ------------------------------------------------------------ */
    public Request2(AbstractHttpConnection connection)
    {
...
loop: for (int i = hostPort.putIndex(); i-- > hostPort.getIndex();)
    ...
    private static final boolean JETTY_SESSIONS_DISABLED=Boolean.getBoolean("JETTY_SESSIONS_DISABLED");
    ...
    {
        if (JETTY_SESSIONS_DISABLED)
        {
            if (create)
            {
                //So we can trace back and find what is causing errant session creations...
                throw new IllegalStateException("sessions are disabled via JETTY_SESSIONS_DISABLED (reh)");
            }
            return null;
        }
    ...
    public boolean isSecure()
    {
        ...
        Object runtimeCast=this;
        return _connection.isConfidential((Request)runtimeCast);
    }
    ...
    public void mergeQueryString(String query)
    {
        ...
        if (_parameters != null && _parameters.size() > 0)
        {
            ...
            Iterator<Map.Entry<String, Object>> iter = _parameters.entrySet().iterator();
            while (iter.hasNext())
            {
        ...
        if (_queryString != null && _queryString.length() > 0)
        {
            if (merge_old_query)
            {
                ...
                Iterator<Map.Entry<String, Object>> iter = overridden_old_query.entrySet().iterator();
                while (iter.hasNext())
                {
                    Map.Entry<String, Object> entry = iter.next();
                    String name = entry.getKey();
